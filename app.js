...
(function() {
  var GameState, Strategy, ThreesState, ThreesState0, ThreesState1, ThreesState2, Value, addValues, bestAction, bestMove, canPushArrayIndex, count, currentTime, dirReverses, dirTransposes, dirs, doFor, gridHeight, gridSize, gridWidth, heuristic, move, pushArray, rollOutEval, sample, strategy, sum, transform, uct, zipWith,
    __slice = [].slice,
  ...
  zipWith = function() {
    ...
    return _.map(f, _.zip.apply(_, lists));
  };
  ...
  Strategy = (function() {
    ...
    Strategy.prototype.initChildren = function(node) {
      var nextStates, states, _ref, _ref1;
      nextStates = node.state.nextStates();
      if (node.state.isRandom) {
        _ref = _.zip(nextStates), node.weights = _ref[0], states = _ref[1];
      } else {
        _ref1 = _.zip(nextStates), node.actions = _ref1[0], states = _ref1[1];
      }
      return node.children = _.map(this.initNode, states);
    };

    Strategy.prototype.explore = function(node) {
      var child, index, state, value;
      state = node.state;
      if (state.terminal) {
        value = this.heuristic(state);
      } else if (node.children === null) {
        this.initChildren(node);
        value = addValues((function() {
        ...
        index = state.isRandom ? sample(node.weights) : this.pick(node);
  ...
  rollOutEval = function(depth, state) {
    ...
    _ref = _zip(state.nextStates()), weights = _ref[0], states = _ref[1];
    if (state.isRandom) {
  ...
  canPushArrayIndex = function(array, index) {
    switch (array[i]) {
      case -1:
        return false;
      case 0:
  ...
  ThreesState0 = (function(_super) {
    ...
    ThreesState0.prototype.terminal = false;
    ...
    ThreesState0.prototype.nextStates = function() {
  ...
  ThreesState1 = (function(_super) {
    ...
    function ThreesState1(grid, free, next) {
      ...
      this.free = free;
      this.next = next;
      this.terminal = this.free.length === 0;
    }
    ...
    ThreesState1.prototype.nextStates = function() {
      ...
      return [
        (function() {
          ...
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            index = _ref[_i];
            _results.push([weight, spawn(index)]);
          }
          return _results;
        }).call(this)
      ];
  ...
  ThreesState2 = (function(_super) {
    ...
    ThreesState2.prototype.spawn = function(val) {
      ...
      row = grid[index].slice(0);
      row[0] = val;
      grid[index] = row;
      return new ThreesState0(grid, null);
    };

    ThreesState2.prototype.nextStates = function() {
      var val, vals, weight;
      vals = this.next === null ? [1, 2, 3] : this.next;
      ...
      return [
        (function() {
          ...
          for (_i = 0, _len = vals.length; _i < _len; _i++) {
            val = vals[_i];
            _results.push([weight, spawn(val)]);
          }
          return _results;
        })()
      ];
